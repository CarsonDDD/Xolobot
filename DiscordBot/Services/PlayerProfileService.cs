...
public class PlayerProfileService(DatabaseService db)
{
    ...
    public PlayerProfile? GetProfile(int playerId)
    {
        ...
        var stats = conn.Query<Stat>(
                "SELECT * FROM Stat WHERE player_id = @id",
                new { id = playerId }
            )
            .ToList();
        ...
        if (inventory != null)
        {
            var inventoryItems = conn.Query<InventoryItem>(
                    "SELECT * FROM InventoryItem WHERE inventory_id = @invId",
                    new { invId = inventory.Id }
                )
                .ToList();
            ...
            foreach (var invItem in inventoryItems)
            {
                ...
                displayList.Add(new ItemStack { DbMeta = invItem, Item = itemWithTags });
            }
        }
        ...
        return new PlayerProfile
        {
            ...
            Inventory =
                inventory == null
                    ? null
                    : new InventoryWithItems { Inventory = inventory, Items = displayList },
        };
    }
